...
from config import HYPERCUBE_SIZE, APP_NAME
...
class Hypercube:
    def __init__(self):
        log.info("Initiating lattice with the following labels", LABELS)
        self.graph = nx.relabel_nodes(
            nx.generators.lattice.hypercube_graph(HYPERCUBE_SIZE), LABELS)

    def get_shortest_path(self, source, target):
        log.info("Finding shortest path between", source, "and", target)
